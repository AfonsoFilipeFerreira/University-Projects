...
// Check if the user submitted a new profile picture
if (isset($_FILES['profile_picture'])) {
  $username = $_SESSION["username"];
  $target_dir = $profilePictureDir;
  $target_file = $target_dir . $username . "_" . basename($_FILES["profile_picture"]["name"]);

  // Move the uploaded file to the target directory
  move_uploaded_file($_FILES["profile_picture"]["tmp_name"], $target_file);

  // Update the user's profile picture in the session
  $_SESSION[$username]["profile_picture"] = $target_file;

  $profilePicture = $target_file;
} else {
  // Check if the user has a profile picture in the session
  $username = $_SESSION["username"];
  ...
  // If no file with the username is found, use the default profile picture
  if ($profilePicture == '') {
    $profilePicture = $profilePictureDir . $defaultProfilePicture;
  }
}

// Retrieve approved publications for the current user
$sql = "SELECT * FROM publication WHERE aproved = 1 AND username = '$username' ORDER BY id DESC";
  ...
  <main class="main-content">
    <div class="profile-section">
      ...
      <form action="" method="POST" enctype="multipart/form-data">
        <input type="file" class="custom-button" name="profile_picture" accept="image/*"> <br>
        <input type="submit" class="custom-button" value="Change Profile Picture">
      </form>
    </div>
        ...
        <div class="publication">
          ...
          <div class="publication-content">
            ...
            <div class="pub-interactions">
              <?php if ($userLiked): ?>
                <form action="unlikeprofile.php?username=<?php echo $username; ?>" method="POST">
                  <input type="hidden" name="username" value="<?php echo $username; ?>">
                  <input type="hidden" name="publication_id" value="<?php echo $row['id']; ?>">
                  <button type="submit" name="unlike" class="like" data-publication-id="<?php echo $row['id']; ?>">
                    <img src="liked_button-removebg-preview.png" alt="Like" class="like">
                  </button>
                </form>
              <?php else: ?>
                <form action="likeprofile.php?username=<?php echo $username; ?>" method="POST">
                  <input type="hidden" name="username" value="<?php echo $username; ?>">
                  <input type="hidden" name="publication_id" value="<?php echo $row['id']; ?>">
            ...
            <div class="comments-session">
              <div>
                <form action="add_comment_profile.php?username=<?php echo $username; ?>" method="POST"
                  class="comment-form">
                  <input type="hidden" name="publication_id" value="<?php echo $row['id']; ?>">
            ...
            <?php if ($username === $row['username']): ?>
              <form action="delete_pub_profile.php" method="POST" class="delete-publication-form">
                <input type="hidden" name="username" value="<?php echo $username; ?>">
                <input type="hidden" name="publication_id" value="<?php echo $row['id']; ?>">
